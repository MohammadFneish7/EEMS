...
Public Class frmItemsConsumption
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles btnadd.Click
        Dim frm As New frmItemsConsumptionEditor
    ...
    Private Sub loadData()
        ...
        If chkSelectAll.Checked Then
            a.GetData("SELECT c.ID as [المعرّف],[outdate] as [تاريخ الاستهلاك],[itemname] as [اسم الصنف],[quantity] as [الكمّيّة], detioration as [للتلف؟], [partyname] as [الجهة],[partyPhone] as [هاتف الجهة],c.notes as [ملاحظات] FROM Consumption c JOIN Items i on c.itemid=i.id ORDER BY outdate DESC")
        Else
            a.GetData("SELECT c.ID as [المعرّف],[outdate] as [تاريخ الاستهلاك],[itemname] as [اسم الصنف],[quantity] as [الكمّيّة], detioration as [للتلف؟], [partyname] as [الجهة],[partyPhone] as [هاتف الجهة],c.notes as [ملاحظات] FROM Consumption c JOIN Items i on c.itemid=i.id where outdate >='" & dtp1.Value.ToShortDateString & "' and outdate <='" & dtp2.Value.ToShortDateString & "' ORDER BY outdate DESC")
        End If
        ...
        Try
            ...
            If dr = Windows.Forms.DialogResult.Yes Then
                ...
                If todeleteIds.Length > 1 Then
                    ...
                    a.Execute("DELETE FROM Consumption Where ID in " & todeleteIds)
                End If
