...
Public Class frmFuelTanks
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles btnadd.Click
        Dim frm As New frmFuelTankEditor(0, True)
    ...
    Private Sub loadData()
        a.ds = New DataSet
        a.GetData("SELECT ID as [المعرف],tankName as [اسم الخزّان],location as [الموقع],capacity as [السعة],notes as [ملاحظات] FROM FuelTank")
    ...
    Private Sub buttonEditPressed(sender As Object, e As ButtonPressedEventArgs)
        If GridView1.GetSelectedRows.Count > 0 Then
            Dim frm As New frmFuelTankEditor(GridView1.GetRowCellValue(GridView1.GetSelectedRows(0), GridView1.Columns(1)), False)
            Dim dr As DialogResult = frm.ShowDialog
            If dr =DialogResult.OK Then
                loadData()
            End If
        ...
        Try
            ...
            If dr =DialogResult.Yes Then
                ...
                For Each row As Integer In listToRemove
                    a.Execute("DELETE FROM FuelTank Where ID=" & GridView1.GetRowCellValue(row, GridView1.Columns(1)).ToString)
                Next
