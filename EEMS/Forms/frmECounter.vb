...
Public Class frmECounter
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles btnadd.Click
        Dim frm As New frmECounterEditor(0, True, 0)
    ...
    Private Sub loadData()
        a.ds = New DataSet
        a.GetData("SELECT c.ID as المعرّف,c.serial as [رقم العداد],c.code as [الرمز في العلبة],b.code as [رمز العلبة],b.location as [عنوان العلبة],c.active as [مفعّل], isNull(c.currentvalue,0) as [القيمة الأخيرة],c.notes as ملاحظات FROM ECounter c Join ElectricBox b ON c.boxid=b.id")
    ...
    Private Sub buttonReportPressed(sender As Object, e As ButtonPressedEventArgs)
        If GridView1.GetSelectedRows.Count > 0 Then
            Dim frm As New frmECounterEditor(GridView1.GetRowCellValue(GridView1.GetSelectedRows(0), GridView1.Columns(1)), False, GridView1.GetRowCellValue(GridView1.GetSelectedRows(0), GridView1.Columns(7)))
            Dim dr As DialogResult = frm.ShowDialog
            If dr =DialogResult.OK Then
                loadData()
            End If
        ...
        Try
            ...
            If dr =DialogResult.Yes Then
                ...
                For Each row As Integer In listToRemove
                    a.Execute("DELETE FROM ECounter Where ID=" & GridView1.GetRowCellValue(row, GridView1.Columns(1)).ToString)
                Next
    ...
    Private Sub ECounter_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
