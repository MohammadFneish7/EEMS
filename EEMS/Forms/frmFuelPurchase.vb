...
Public Class frmFuelPurchase
    ...
    Private Sub loadData()
        ...
        If chkSelectAll.Checked Then
            a.GetData("SELECT fp.ID as [المعرّف],indate as [تاريخ الشراء],brand as [الماركة],quantity as [الكمّيّة],pricetotal as [اجمالي قيمة الشراء ل.ل],partyname as [الجهة],partyPhone as [هاتف الجهة],tankname as [اسم الخزّان],fp.notes as [ملاحظات] FROM FuelPurchases fp join FuelTank ft on fp.tankid = ft.id ORDER BY indate DESC")
        Else
            a.GetData("SELECT fp.ID as [المعرّف],indate as [تاريخ الشراء],brand as [الماركة],quantity as [الكمّيّة],pricetotal as [اجمالي قيمة الشراء ل.ل],partyname as [الجهة],partyPhone as [هاتف الجهة],tankname as [اسم الخزّان],fp.notes as [ملاحظات] FROM FuelPurchases fp join FuelTank ft on fp.tankid = ft.id where indate >='" & dtp1.Value.ToShortDateString & "' and indate <='" & dtp2.Value.ToShortDateString & "' ORDER BY indate DESC")
        End If
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If Not currentUser.hasPermision("deleteFuelPurchase") Then
