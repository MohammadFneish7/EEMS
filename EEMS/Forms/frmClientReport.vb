...
Public Class frmClientReport
    ...
    Private Sub loadData()
        ...
        a.GetData("SELECT ch.ID as [معرّف القيمة],r.ID as [معرّف الاشتراك],(ar.caption + '-' + CAST(ch.cyear as nvarchar(50))) as [شهر],ch.monthlyfee as [رسم اشتراك ل.ل],(ch.currentvalue-ch.previousvalue) as [فرق عداد (كيلو)],ch.kilowattprice as [سعر الكيلو ل.ل],((ch.currentvalue-ch.previousvalue)*ch.kilowattprice)+roundvalue as [مطلوب كيلو ل.ل],total+discount as [المجموع ل.ل],ch.discount as [حسم ل.ل],total as [مطلوب ل.ل],dollarprice as [سعر الصرف تاريخه], ISNULL(sum(Cast(p.pvalue AS BIGINT)),0) as [مدفوع ل.ل], total-ISNULL(sum(Cast(p.pvalue AS BIGINT)),0) as [باقي ل.ل] " &
                    ...
                    " group by ch.ID,r.ID,(ar.caption + '-' + CAST(ch.cyear as nvarchar(50))),ch.monthlyfee,(ch.currentvalue-ch.previousvalue),ch.kilowattprice,((ch.currentvalue-ch.previousvalue)*ch.kilowattprice)+roundvalue,total+discount,ch.discount,ch.total,ch.dollarprice", "dt2")
