...
Public Class frmChangeMonthlyFee

    Dim a As New Helper

    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click

        If Not String.IsNullOrWhiteSpace(txtmonthlyfee.Text) > 0 And (CheckBox1.Checked Or Not String.IsNullOrWhiteSpace(txtpackid.Text)) Then
            ...
            End If
            a.Execute("Update CounterHistory set monthlyfee=" & txtmonthlyfee.Text.Trim & " where " & conditionQuery)
            a.Execute("Update CounterHistory set roundvalue=IIF((1000 - (([monthlyfee]+([kilowattprice])) % 1000))=1000,0,(1000 - (([monthlyfee]+([kilowattprice])) % 1000))) where " & conditionQuery)

            MsgBox("تم تعديل رسوم الاشتراكات.")
        Else
            MsgBox("الرجاء تعبئة كل الخانات المطلوبة للمتابعة.")
        End If
    End Sub

    Private Sub txtkiloprice_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtmonthlyfee.KeyPress
        a.bindNumeric(sender, e)
    End Sub

    Private Sub frmChangeMonthlyFee_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
