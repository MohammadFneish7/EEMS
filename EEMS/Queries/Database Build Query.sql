...
CREATE TABLE CounterHistory (ID INT IDENtity(1,1) PRIMARY KEY,cmonth INT NOT NULL,cyear INT NOT NULL,regid INT DEFAULT 0 NOT NULL,monthlyfee INT DEFAULT 0,kilowattprice INT DEFAULT 0,previousvalue INT DEFAULT 0,currentvalue INT DEFAULT 0,roundvalue INT DEFAULT 0,dollarprice INT DEFAULT 1500,discount INT DEFAULT 0,total as [monthlyfee]+([kilowattprice]*([currentvalue]-[previousvalue]))+[roundvalue]-[discount],totaldollar AS CAST(CAST(([monthlyfee]+([kilowattprice]*([currentvalue]-[previousvalue]))+[roundvalue]-[discount]) AS FLOAT)/CAST([dollarprice] AS FLOAT) AS DECIMAL(18,2)),whatsappmsgsent BIT NOT NULL DEFAULT 0,notes VARCHAR(255));
...
CREATE TABLE Expenditure (ID INT IDENtity(1,1) PRIMARY KEY,expdate date NOT NULL,title VARCHAR(50),amount INT DEFAULT 0,amount_dollar DECIMAL(18,2) NOT NULL DEFAULT 0,currency BIT  NOT NULL DEFAULT 0,party VARCHAR(50),detail VARCHAR(255),paymentRef VARCHAR(50));
